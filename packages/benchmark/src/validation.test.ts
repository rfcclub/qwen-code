...
describe('validateResult', () => {
  ...
  it('validates custom async function', async () => {
    const result = await validateResult({
      type: 'custom',
      params: {
        fn: async () => false,
  ...
  it('throws when custom fn is missing', async () => {
    await expect(
      validateResult({
        type: 'custom',
        params: {},
      }),
    ).rejects.toThrow('Custom validation requires a fn parameter');
  });

  it('throws on unknown validation type', async () => {
    await expect(
      validateResult({
        type: 'unknown_type' as unknown as 'file_exists',
        params: {},
      }),
